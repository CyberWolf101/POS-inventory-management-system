...
function SalesRepActionsModal({ isOpen, onClose, theRep, inventoryData, handleGetInventoryData }) {
    ...
    useEffect(() => {
        setEmail(theRep.email)
        setPhone(theRep.phone)
    }, [theRep])
    ...
    const handleDelete = () => {
        const idToRemove = theRep.id;
        const theArray = inventoryData.salesRep;
        const docRef = doc(db, 'inventory_data', inventoryData.id)

        swal({
            title: 'Are You sure?',
            text: "Delete this sales rep?",
            icon: "warning",
            buttons: true,
            dangerMode: true,
            ...
            if (willDelete) {
                setLaoding(true)
                const filteredArray = theArray.filter((rep) => rep.id !== idToRemove);
                await updateDoc(docRef, {
                    salesRep: filteredArray
                })
                toast({
                    title: 'sales rep was deleted',
                    status: 'info',
                    variant: 'subtle',
                    duration: 2000,
                    position: 'top',
            ...
            <Modal isOpen={isOpen} onClose={onClose} >
                ...
                <ModalContent style={{ marginTop: '200px' }}>
                    <center className='modalHeader'>
                        <ModalHeader><h6>{theRep.name}</h6></ModalHeader>
                        <ModalCloseButton />
                    </center>
                    <ModalBody className='' >
                        <center>
                            <div className='supplier-actions-btns'>
                                ...
                                <a className='btn btn-custom '
                                    href={'tel:' + theRep.phone}
                                    
                                >
                                    Call
                                    <Phone fontSize='small' /></a>
...
export default SalesRepActionsModal;